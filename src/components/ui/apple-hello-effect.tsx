...
type Props = ComponentProps<typeof motion.svg> & {
  ...
  /** Skip stroke draw (already completed) — for follow-up screens. */
  drawn?: boolean
}
...
export function AppleHelloVietnameseEffect({
  ...
  return (
    <motion.svg
      className={cn('h-20 w-auto max-w-[min(100vw-2rem,56rem)]', className)}
...
export function AppleHelloEnglishEffect({
  ...
  const calc = (x: number) => x * speed
  const pathInitial = drawn ? animateProps : initialProps
  const pathTransition = (d: number, delay: number, op: number) =>
    drawn
      ? { duration: 0 }
      : {
          duration: calc(d),
          ...
          delay: calc(delay),
          opacity: { duration: op, delay: calc(delay) },
        }

  return (
    <motion.svg
      className={cn('h-20 w-auto max-w-[min(100vw-2rem,40rem)]', className)}
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 638 200"
      fill="none"
      stroke="currentColor"
    ...
    >
      ...
      <motion.path
        ...
        initial={pathInitial}
        animate={animateProps}
        transition={pathTransition(2.8, 0.7, 0.7)}
        onAnimationComplete={drawn ? undefined : onAnimationComplete}
      />
    </motion.svg>
  )
}
